...
public class PopupSound : StrictPopup {
    ...
    [SerializeField, ReadOnly] private string audioClipDirectory = "Assets/Audio/";
    private readonly string[] EXTENSIONS = { ".wav", ".mp3", ".flac" };
    ...
    public void SetKey(Sound key) => this.key = key;
    ...
    public void TryAutoAssignClip() {
        ...
        string[] clipExtensions = EXTENSIONS;

        audioSource = GetComponent<AudioSource>();
        string extension = "";
        AudioClip found = null;

        foreach (string ex in clipExtensions) {

            found = AssetDatabase.LoadAssetAtPath<AudioClip>(audioClipDirectory + gameObject.name + ex);
        ...
        if (found == null)
            print("Could not find file " + audioClipDirectory + gameObject.name);
        else {

            audioSource.clip = found;
            ForceSetDuration();
            print("Clip assigned succesfully (" + audioClipDirectory + gameObject.name + extension + ")");
