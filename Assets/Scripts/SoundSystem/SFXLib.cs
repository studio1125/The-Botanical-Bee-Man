...
namespace SFX {
    ...
    // Use SoundType to organize in inspector
    public enum Sound {
...
public class SFXLib : MonoBehaviour {
    ...
    public void ValidateDict(bool modifyDict, bool printSuccessMessage) {
        ...
        if (modifyDict) {
            ...
            Sound[] allSounds = (Sound[])Enum.GetValues(typeof(Sound));

            //all entries already listed in existingEntries
            HashSet<Sound> existingEntries = soundDict.Select(entry => entry.Key).ToHashSet();
        ...
        HashSet<Sound> seenKeys = new HashSet<Sound>();
        Dictionary<Sound, string> duplicateKeyLog = new Dictionary<Sound, string>();
        ...
        if (!hasErrors && printSuccessMessage)
            ...
            foreach (Sound key in duplicateKeyLog.Keys)
    ...
    public PopupSound GetPopup(Sound key) {
    ...
    public Sound? GetKey(PopupSound popup) {
